...
def predict_depths(wavenums, spectrum_file, Mp, Rp, T0, mass_loss_rate, Rs, D_over_a, max_r_over_Rp=9.9):
    r_mesh, v_interp, n3_interp = get_solution(spectrum_file, Mp, Rp, T0, mass_loss_rate, D_over_a)
...
print("Rate", 21.6 * parsec / (0.08 * AU))

#GJ 436b
transit_spectrum = predict_depths(wavenums, sys.argv[1], 0.073 * M_jup, 0.38 * R_jup, 5000, 2e10, 0.464 * R_sun, (9.76 * parsec / 0.03 / AU))

#HD 97658b
#transit_spectrum = predict_depths(wavenums, sys.argv[1], 0.03 * M_jup, 0.21 * R_jup, 3000, 3e9, 0.74 * R_sun, (21.6 * parsec / 0.08 / AU), 9.9)
